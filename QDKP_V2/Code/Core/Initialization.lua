...
function QDKP2_Enable_MSChanges()
	if not MSChangesAvailable then
...
function QDKP2_MS_Pattern_Matcher(text, class)
	local commandPatterns = {
		...
		{pattern='^fro%a*d%a*',token='Frost DPS'},
		...
		{pattern='^cat%a*',token='Feral DPS'},
		{pattern='^bear%a*',token='Feral Tank'},
		{pattern='^gua%a*',token='Feral Tank'},
		{pattern='^fe%a*d%a*',token='Feral DPS'},
		{pattern='^fe%a*t%a*',token='Feral Tank'},
		{pattern='^fe%a*',token='Feral'},
		{pattern='^kit%a*',token='Feral DPS'},
